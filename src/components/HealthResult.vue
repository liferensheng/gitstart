    ...
    var that=this;
          // 取消任务完成弹框
           if(localStorage.taskInfo){
             var s=JSON.parse(localStorage.taskInfo);
             that.taskSuccess=Number(s.isFinish)==0;
           }
          //  that.taskSuccess=true;
            localStorage.taskInfo=JSON.stringify({user:getUrlName("user_id"),isFinish:1});
          //  that.taskSuccess=!JSON.parse(localStorage.hasResult);
...
/*任务成功弹框*/
 .mask{
  ...
  .maskMian{
    ...
    box-sizing: border-box;
    padding:.413333rem .866667rem .8rem;
    position: absolute;
  ...
  .taxkOk{
    width: 6.053333rem;
    height: 1.493333rem;
    background: url("../assets/img/meiriyidu_wanchengcaixie_bg@3x.png")center center no-repeat;
    ...
    .maskTitle{
        ...
        margin:0.266666rem 0 0.533333rem;
        text-align: center;
    }
    .goBotton{
        width:6rem;
